    ...
    try:
        ...
        if not company:
            company = Company(
                ...
                annual_revenue_range="1,000〜5,000万円",
            ...
            company.name = company.name or company.company_name or "デモ株式会社"
            company.company_name = company.company_name or company.name or "デモ株式会社"
            company.industry = company.industry or "飲食業"
            ...
            company.annual_revenue_range = company.annual_revenue_range or "1,000〜5,000万円"
            company.location_prefecture = company.location_prefecture or "東京都"
        ...
        if db.query(Memory).filter(Memory.user_id == user.id).count() == 0:
            memory = Memory(
                user_id=user.id,
                current_concerns="資金繰りと売上停滞が気になる",
                important_points="採用強化と販路拡大が必要",
                remembered_facts="テイクアウト導入済み",
